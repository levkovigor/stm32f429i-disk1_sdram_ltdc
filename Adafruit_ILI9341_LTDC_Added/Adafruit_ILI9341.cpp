...
*/
/**************************************************************************/
/*afruit_ILI9341::Adafruit_ILI9341(tftBusWidth busWidth, int8_t d0, int8_t wr,
...
static const uint8_t PROGMEM initcmdLTDC[] = {
  ILI9341_DISPOFF, 0x80,
  0xCF, 3, 0x00, 0xC1, 0x30,
  0xED, 4, 0x64, 0x03, 0x12, 0x81,
  0xE8, 3, 0x85, 0x00, 0x78,
  ...
  ILI9341_VMCTR1  , 2, 0x45, 0x15,       // VCM control
  ILI9341_VMCTR2  , 1, 0x90,             // VCM control2
  ILI9341_MADCTL  , 1, 0xC8,   
  0xF2, 1, 0x00,                         // 3Gamma Function Disable
  0xB0   , 1, 0xC2,
  ...
  ILI9341_RAMWR  , 0x80, 
  ILI9341_GAMMASET , 1, 0x01,             // Gamma curve selected
  ILI9341_GMCTRP1 , 15, 0x0F, 0x29, 0x24, 0x0C, 0x0E, 0x09, // Set Gamma
    0x4E, 0x78, 0x3C, 0x09, 0x13, 0x05, 0x17, 0x11, 0x00,
  ILI9341_GMCTRN1 , 15, 0x00, 0x16, 0x1B, 0x04, 0x11, 0x07, // Set Gamma
    0x31, 0x33, 0x42, 0x05, 0x0C, 0x0A, 0x28, 0x2F, 0x0F,
  ILI9341_SLPOUT  , 0x80,                // Exit Sleep
  ILI9341_DISPON  , 0x80,                // Display on     
  ILI9341_RAMWR  , 0x80,   
  0x00                                   // End of list
...
void Adafruit_ILI9341::begin_ltdc(uint32_t freq) {
  ...
  uint8_t cmd, x, numArgs;
  const uint8_t *addr = initcmdLTDC;
  while ((cmd = pgm_read_byte(addr++)) > 0) {
    x = pgm_read_byte(addr++);
    numArgs = x & 0x7F;
    sendCommand(cmd, addr, numArgs);
